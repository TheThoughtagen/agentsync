...
- [x] **INIT-01**: `aisync init` completes with zero drift — `aisync status` shows all tools OK immediately after init
- [x] **INIT-02**: `aisync status` only shows tools that are configured in `aisync.toml` or detected, not all possible adapters
- [x] **INIT-03**: `aisync init` presents interactive source tool selection when multiple instruction sources exist
